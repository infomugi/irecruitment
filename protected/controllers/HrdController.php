...
class HrdController extends Controller
{
	...
	{
		$dataProvider=new CActiveDataProvider('Hrd');
	...
	{
		$model=new Hrd('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Hrd']))
			$model->attributes=$_GET['Hrd'];
	...
	{
		$model=Hrd::model()->findByAttributes(array('id_user'=>$id));
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	}		
	 ...
	 * @param Hrd $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='Hrd-form')
		{
